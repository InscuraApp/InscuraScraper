MODULE := inscurascraper

SERVER_NAME := inscurascraper-server
...
BUILD_COMMIT  := $(if $(BUILD_COMMIT),$(BUILD_COMMIT),unknown)
BUILD_VERSION := $(shell git describe --abbrev=0 --tags --match 'v[0-9]*' HEAD 2>/dev/null | sed 's/^v//')
BUILD_VERSION := $(if $(BUILD_VERSION),$(BUILD_VERSION),0.0.1)
